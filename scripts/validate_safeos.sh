...
if [[ ! -f "$ROOT_DIR/iso/safeos-amd64.iso" ]]; then
  echo "Missing ISO artifact: $ROOT_DIR/iso/safeos-amd64.iso"
  exit 1
fi

if [[ ! -f "$ROOT_DIR/iso/safeos-amd64.sha256" ]]; then
  echo "Missing checksum artifact: $ROOT_DIR/iso/safeos-amd64.sha256"
  exit 1
fi
